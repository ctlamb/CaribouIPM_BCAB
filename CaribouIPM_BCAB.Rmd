...
##Herd Abundance
ggplot() +
  geom_ribbon(data=demog%>%filter(herd%in%"Klinse-Za")%>%mutate(herd="1.Subpopulation (X%)"),
              aes(x=yrs, y=totNMF, ymin=totNMF.lower, ymax=totNMF.upper),alpha=0.4, color=NA, fill=cols[3])+
  geom_line(data=demog%>%filter(herd%in%"Klinse-Za")%>%mutate(herd="1.Subpopulation (X%)"),
            aes(x=yrs, y=totNMF, ymin=totNMF.lower, ymax=totNMF.upper),size=1, color=cols[3]) +
  geom_rug(data=rbind(
    afr%>%select(herd,year,est)%>%mutate(type="Recruit"),
    afs%>%select(herd,year,est)%>%mutate(type="Surv"),
  ...
  theme(axis.title.x = element_text(size=15),
        ...
        legend.text = element_text(size=13),
        legend.title=element_text(size=15))+
  geom_point(data=counts%>%filter(herd%in%"Klinse-Za")%>%mutate(herd="1.Subpopulation (X%)"),
             aes(x=year, y=Est_CL),size=0.5, alpha=0.7)+
  geom_linerange(data=counts%>%filter(herd%in%"Klinse-Za")%>%mutate(herd="1.Subpopulation (X%)"),
                 aes(x=year, ymin=Est_CL.min,  ymax=Est_CL.max),alpha=0.5)+
  geom_point(data=trt.plot%>%filter(herd%in%"Klinse-Za")%>%mutate(herd="1.Subpopulation (X%)"),
             aes(x=year, y=y, group=treatment),size=0.5)+
  scale_color_manual(values=cols[-4])+
  geom_text(data = trt.plot%>%
                ungroup%>%
