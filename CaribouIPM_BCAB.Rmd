...
## Data housekeeping
```{r Data housekeeping, message=FALSE, warning=FALSE}
...
```{r Firstyr, message=FALSE, warning=FALSE}
raw.demog <- rbind(
...
## Data export for folks that want the raw data
```{r data export, message=FALSE, warning=FALSE}
## pull posterior draws, add in herd, year, and treatment to each herd-year
demog.export.raw <- out %>%
  spread_draws(
    c(totNMF, totN, totAdults, totCalves, totAdultsMF, totCalvesMF, S, R_adj, lambda, SR)[i, j],
    ndraws = ndraws
  ) %>%
  median_qi(.width = 0.9) %>%
  left_join(yr_df %>% rename(j = yr_idx), by = "j") %>%
  ...
  mutate(trt = replace_na(trt, "Reference")) %>%
  ungroup() %>%
  rename_at(vars(contains("R_adj")), ~ str_replace_all(., "R_adj", "recruitment.per.adult")) %>%
  rename_at(vars(contains("S")), ~ str_replace_all(., "S", "survival"))%>%
  rename_at(vars(contains("totNMF")), ~ str_replace_all(., "totNMF", "totPopMF"))%>%
  rename_at(vars("totAdults"), ~ str_replace_all(., "totAdults", "totAdultsF"))%>%
rename_at(vars("totCalves"), ~ str_replace_all(., "totCalves", "totCalvesF"))%>%
rename_at(vars("totN"), ~ str_replace_all(., "totN", "totPopF"))%>%
rename_at(vars(contains("totAdults.")), ~ str_replace_all(., "totAdults.", "totAdultsF."))%>%
rename_at(vars(contains("totCalves.")), ~ str_replace_all(., "totCalves.", "totCalvesF."))%>%
rename_at(vars(contains("totN.")), ~ str_replace_all(., "totN.", "totPopF."))%>%
  select(herd,
  year = yrs, 
  recovery.action.yrplus1 = trt,
  lambda:.interval
)


## filter herds to a period where not fully extirpated
extirpated.yr.export <- demog.export.raw %>%
  filter(totPopMF <= 1) %>%
  group_by(herd) %>%
  filter(year == min(year)) %>%
  ungroup() %>%
  select(herd, year, totPopMF)

write_csv(extirpated.yr.export, "tables/clean/extirpated.year.csv")


fn.extirpated.yr.export <- demog.export.raw %>%
  filter(round(totAdultsF, 0) < 10 | round(totPopMF, 0) < 20) %>%
  group_by(herd) %>%
  filter(year == min(year)) %>%
  ungroup() %>%
  select(herd, year, totAdultsF, totPopMF)

write_csv(extirpated.yr.export, "tables/clean/fn.extirpated.year.csv")


herds.export <- unique(demog.export.raw$herd)

demog.export.trim <- tibble()
for (i in 1:length(herds.export)) {
  if (herds[i] %in% extirpated.yr.export$herd) {
    yr <- extirpated.yr.export %>%
      dplyr::filter(herd == !!herds[i])
    a <- demog.export.raw %>%
      dplyr::filter(herd == !!herds[i] & year <= yr$year)
  }
  ...
  ##denote years with demography data
   yr.data <- first.yr %>%
      dplyr::filter(herd == !!herds[i])
  a <- a %>%
      mutate(quality=case_when( year < yr.data$first.year ~ "low",
                                year >= yr.data$first.year ~ "good"
                                ))

demog.export.trim <- bind_rows(a, demog.export.trim)
rm(a)
rm(yr)
rm(yr.data)
}

demog.export.raw <- demog.export.trim
rm(demog.export.trim)


write_csv(demog.export.raw, "tables/clean/SMC_IPM_Demography_posteriors.csv")
```
...
## Plot Abundance by Herd
```{r Plot herd abundance split, message=FALSE, include=FALSE}
## split into two
## TOP
ggplot() +
  geom_ribbon(
...
```{r Plot total abundance, fig.height=6, fig.width=6, message=FALSE, warning=FALSE}
...
sims.plot %>%
  mutate(.variable = case_when(
    .variable == "totNMF" ~ "With recovery actions",
    TRUE ~ "Status quo"
  )) %>%
  write_csv("tables/pop.ests.csv")
...
## Gather draws
demog.draws <- out %>%
  gather_draws(logla[i, j], S[i, j], R_adj[i, j], totNMF[i, j], totN[i, j], sexratio[i, j], totAdults[i, j], totCalves[i, j], totAdultsMF[i, j], totCalvesMF[i, j], ndraws = ndraws) %>% ## for Tal
...
demog.draws %>% write_csv(here::here("tables", "draws", "demog.draws_forTal.csv"))

# ###how to calculate from draws
# lambda.compare <- demog%>%
#   ungroup%>%
#   mutate(r=log(lambda))%>%
#   dplyr::select(herd,yrs,r)%>%
#   left_join(demog.draws %>%
#               group_by(herd,yrs)%>%
#               summarise(median=median(r),
#                         mean=mean(r)))
#
# lm(r~median, data=lambda.compare)%>%summary
# lm(r~mean, data=lambda.compare)%>%summary
#
...
```{r trt eff- BA, fig.height=7, fig.width=10, message=FALSE, warning=FALSE}
# Before-After ---------------------------------------------
...
```{r application, message=FALSE, warning=FALSE}
...
```{r ind trt eff, message=FALSE, warning=FALSE}
...
## Map
```{r map, fig.height=8, fig.width=12, message=FALSE, warning=FALSE}
## Prep Herd Bounds
