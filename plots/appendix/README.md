...
ggplot(first.yr, aes(x = first.year, y=cumsum)) +
  ...
  labs(
    ...
    title = "First year of demographic data for the 41 SMC herds between 1973-2021",
...
| herd      | ECCC           | first.year | count | cumsum |      prop |
|:----------|:---------------|-----------:|------:|-------:|----------:|
...
abundance.all.plot <- ggplot(data = sims.plot %>%
  ...
  geom_text(data = sims.plot %>% filter(yrs == 2021) %>%
    mutate(.variable = case_when(
      .variable == "totNMF" ~ "Observed",
      TRUE ~ "Status quo"
...
cosewic.abund%>%
  ...
  dplyr::select(ECCC, `1993`,`1996`, `2012`, `2021`)%>%
  mutate(decline.10yr=(((`2021`-`2012`)/`2012`)*100)%>%round(0),
         decline.3generation=case_when(ECCC!="Central Group"~(((`2021`-`1993`)/`1993`)*100)%>%round(0),
                                       ECCC=="Central Group"~(((`2021`-`1996`)/`1996`)*100)%>%round(0)))%>%
    gt()%>%
...
for(i in 1:length(herds)){
  ...
  ggplot() +
    ...
    geom_text_repel(data =  trt.dat.i, aes(label = treatment, colour = treatment, x = 2021, y = y*0.97), 
                    direction = "y",
                    seed=999,
                    force=0.5,
                    nudge_x = 20,
  ...
  ggsave(here::here("plots","with_treatments", paste0(herds[i]%>%str_replace_all("[:punct:]", " "),".png")), width=4,height=4, bg="white")
  ...
  ggsave(here::here("plots","without_treatments", paste0(herds[i]%>%str_replace_all("[:punct:]", " "),".png")), width=4,height=4, bg="white")
  ...
  ggsave(here::here("plots","without_rug or treatment", paste0(herds[i]%>%str_replace_all("[:punct:]", " "),".png")), width=4,height=4, bg="white")
  
}
```

## Summarize herd-years and delta r by treatment type, intensity, and ecotype
...
## Population growth Before-After-Control-Impact w/ two alternate ecotype groupings
...
![](README_files/figure-gfm/trt%20eff-%20BACI%20w%20ECCC%20ecotype-1.png)<!-- -->
...
![](README_files/figure-gfm/trt%20eff-%20BACI%20w%20ECCC%20ecotype-2.png)<!-- -->
...
![](README_files/figure-gfm/trt%20eff-%20BACI%20w%20ECCC%20ecotype-3.png)<!-- -->
...
![](README_files/figure-gfm/trt%20eff-%20BACI%20w%20HV1998%20ecotype-1.png)<!-- -->
...
![](README_files/figure-gfm/trt%20eff-%20BACI%20w%20HV1998%20ecotype-2.png)<!-- -->
...
![](README_files/figure-gfm/trt%20eff-%20BACI%20w%20HV1998%20ecotype-3.png)<!-- -->
...
hv.summary <- baci.hv.draws %>%
  ...
  dplyr::select(Treatment = trt.grp, `Delta r (BACI): Heard Vagt (1998)` = delta.r)
...
eccc.summary <- baci.eccc.draws %>%
  ...
  arrange(-delta.l) %>%
  mutate(delta.r = paste0(delta.l, " (", lower, "-", upper, ")")) %>%
  dplyr::select(Treatment = trt.grp, `Delta r (BACI): ECCC` = delta.r)
...
## Plot raw r and delta r posteriors by ECCC ecotype
...
![](README_files/figure-gfm/r%20and%20delta%20r%20by%20ECCC%20ecotype-1.png)<!-- -->
...
![](README_files/figure-gfm/r%20and%20delta%20r%20by%20ECCC%20ecotype-2.png)<!-- -->

``` r
ggsave(here::here("plots", "appendix", "baci_treatments_ecotype.png"), width = 8, height = 7, bg = "white")
```

## Individual treatment effect results by ECCC ecotype
...
## Individual treatment effect results by Heard and Vagt 1998 ecotype
...
![](README_files/figure-gfm/individual%20by%20HV%20ecotype-1.png)<!-- -->
...
## Plot mean posteriors by ECCC ecotype
...
![](README_files/figure-gfm/raw%20results%20ECCC%20ecotype-1.png)<!-- -->
...
![](README_files/figure-gfm/raw%20results%20ECCC%20ecotype-2.png)<!-- -->
...
![](README_files/figure-gfm/raw%20results%20ECCC%20ecotype-3.png)<!-- -->

``` r
ggsave(here::here("plots", "appendix", "ind.trt_eff_boxplot_ecotype.png"), width = 8, height = 7, bg = "white")
```

## Plot mean posteriors by HV1998 ecotype
...
![](README_files/figure-gfm/raw%20results%20HV1998%20ecotype-1.png)<!-- -->
...
![](README_files/figure-gfm/raw%20results%20HV1998%20ecotype-2.png)<!-- -->
...
![](README_files/figure-gfm/raw%20results%20HV1998%20ecotype-3.png)<!-- -->
