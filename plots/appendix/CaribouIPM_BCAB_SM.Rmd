...
abundance.all.plot <- ggplot(data = sims.plot) +
  ...
  annotate(geom = "text", x = 1985, y = 10500, label = "Extirpation\nevent", hjust = "left") +
  annotate(
...
sims.plot.grp <- sims %>%
  ...
  dplyr::group_by(yrs, .variable, .draw, ECCC) %>%
  dplyr::summarize(
    mean = sum(.value),
    .groups = "drop"
  ) %>%
    dplyr::group_by(yrs, .variable,ECCC) %>%
  dplyr::summarize(
    LCL = quantile(mean, 0.05),
    UCL = quantile(mean, 0.95),
    mean = mean(mean),
  ...
  ungroup() %>%
  mutate(
    .variable = fct_relevel(.variable, "Status quo*", "Observed"),
...
ext.yr.grp <- ext.yr%>%
  select(-mean)%>%
    left_join(ecotype,by="herd") %>%
  left_join(sims.plot.grp %>% filter(.variable %in% "Observed") %>% select(yrs, mean), by="yrs") %>%
  select(herd, yrs, mean, ECCC) %>%
  ungroup() %>%
  mutate(ECCC = fct_relevel(ECCC, "Southern Group", "Central Group", "Northern Group"))


ggplot(data = sims.plot.grp) +
  geom_ribbon(alpha = 0.3, aes(x = yrs, y = mean, ymin = UCL, ymax = LCL, fill = .variable), color = NA) +
  geom_path(size = 1, aes(x = yrs, y = mean, ymin = UCL, ymax = LCL, color = .variable, fill = .variable)) +
  geom_text(data = sims.plot.grp %>% filter(yrs == 2021), aes(label = .variable, colour = .variable, x = Inf, y = mean), hjust = 0) +
  theme_ipsum() +
  theme(legend.position = "none") +
  ylab("") +
  xlab("Year") +
  labs(
    x = "Year", y = "Abundance", title = "SMC Population Trend",
  ...
  geom_curve(
    data = tibble(x = 2002, y = 1000, xend = 2010, yend = 500, ECCC = "Northern Group"),
...
ggplot() +
  ...
  geom_linerange(data = afs %>% mutate(
    ucl = case_when(
      (est + sd) > 1 ~ 1,
      TRUE ~ est + sd
    ),
  ...
  ), aes(x = year, ymin = lcl, ymax = ucl, color = herd), alpha = 0.7)
...
ggplot() +
  ...
  geom_linerange(data = afr %>% mutate(
    ucl = case_when(
      (est + sd) > 1 ~ 1,
      TRUE ~ est + sd
    ),
...
ggplot() +
  ...
  geom_hline(
    data = data.frame(herd = unique(demog$herd)),
...
for (i in 1:nrow(trt.herds)) {
  ...
  if (all(nrow(pre.trt) > 0, nrow(post.trt) > 0, nrow(control) > 0)) {
    a <- rbind(pre.trt, post.trt, control) %>%
      dplyr::mutate(
        herd.grp = trt.herds[i, "herd"][[1]],
        trt.grp = trt.herds[i, "trt"][[1]],
        BA = case_when(
          yrs < trt.herds[i, "trt.start"][[1]] ~ 0,
...
ggplot() +
  ...
  geom_line(data = baci %>%
    ...
    summarise(
      lambda = median(lambda),
      totNMF = median(totNMF)
    ), aes(x = yrs, y = lambda, group = herd), alpha = 0.3) +
  theme_ipsum() +
  ylab("") +
  xlab("Year") +
...
sim.ref <- demog.draws %>%
  ...
  ungroup() %>%
  filter(lambda < quantile(lambda, 0.95), lambda > quantile(lambda, 0.05)) %>%
  sample_n(n.sims) %>%
  pull(lambda)
...
## export for Tal
write_csv(pc %>% filter(
  herd %in% c("A La Peche", "Brazeau", "Maligne", "Tonquin"),
  yrs >= 2000
) %>% select(.draw:intensity), "IPM_posteriors_AB.csv")
```
