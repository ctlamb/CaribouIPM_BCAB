...
![](README_files/figure-gfm/Data%20housekeeping-2.png)<!-- --> \## First
year of demographic data for each herd
...
### Considered only 1991-2021, as most herds don’t have demographic data before 1991,pre-1991 trends for most SMC herds are informed solely by the priors and shared paramaters with other herds

``` r
#### Summarize bou pop in '91 vs 2021####
sims.summary <- sims.plot %>%
  group_by(.variable) %>%
  filter(yrs == 1991 | yrs == 2021) %>%
  arrange(yrs) %>%
  ungroup()
...
p.decline <- sims.summary %>%
  ...
  summarise(dif = ((`1991` - `2021`) / `1991`) * 100) %>%
  round(0)
...
bmap.big <- basemap_raster(
  ext = herd.bounds %>% st_buffer(200000) %>% st_transform(3857),
