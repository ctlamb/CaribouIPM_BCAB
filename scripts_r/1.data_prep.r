...
##     write_csv(counts %>% select(herd, year, count, Sightability, sd, MinUsed, Est_CL) %>% filter(herd %in% herds[i]), here::here("data", "herd_checks", herds[i], "cleaned_data", "counts.csv"))
...
## 
##     recruitment %>%
##       select(herd, year, calves, adults) %>%
##       filter(herd %in% herds[i]) %>%
