...
## Add in 2022+ data
```{r add new data, results='hide', message=FALSE, warning=FALSE}
## update the data
...
##remove QT full for accurate accounting

## herds
nherd-1 ##remove Quintette full

## n abundance estimates
counts.ss <- counts%>%filter(herd!="Quintette Full")
nrow(counts.ss)
length(unique(counts.ss$herd)) ## n herds with counts
sum(counts.ss$MinUsed == 1)
sum(counts.ss$MinUsed != 1)

## n sightability estimates
sight.count <- counts.ss %>% filter(!is.na(Sightability) & MinUsed != 1 & !is.na(CollarsSeen))
nrow(sight.count)
length(unique(sight.count$herd)) ## n herds with sightability

## n recruitment estimates
recruitment.ss <-recruitment%>%filter(herd!="Quintette Full")
nrow(recruitment.ss)
length(unique(recruitment.ss$herd)) ## n herds with recruit

## n survival estimates
surv.yr.est.ss <- surv.yr.est%>%filter(herd!="Quintette Full")
surv.yr.ss <- surv.yr%>%filter(herd!="Quintette Full")
nrow(surv.yr.est.ss)
length(unique(surv.yr.est.ss$herd)) ## n herds with recruit
sum(surv.yr.ss$time) / 365 # n animal-years for survival
sum(surv.yr.ss$event) # n dead
length(surv.yr.ss$id %>% unique()) # n animals for survival

## timeframe
min(counts.ss$year, recruitment.ss$year, surv.yr.ss$year)
